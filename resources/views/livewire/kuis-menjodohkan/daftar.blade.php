<div>
    ...
    @if ($kelases->isNotEmpty())
        @foreach ($kelases as $kelas)
            <h2 class="text-xl font-bold mt-6 mb-3">Kuis untuk Kelas: {{ $kelas->nama }}</h2>

            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                @forelse($kelas->kuisMenjodohkan()->where('status', 'Published')->get() as $kuis)
                    @php
    ...
    @else
        <x-alert title="Anda belum terdaftar di kelas manapun."
            description="Hubungi guru atau admin untuk mendaftarkan Anda ke dalam kelas." />
    @endif
</div>
